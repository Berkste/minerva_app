...
// (it and the keystore are git-ignored). When it is absent — a fresh clone, CI
// without secrets — the release build falls back to the debug key so the build
// still runs. A store-bound build requires the real keystore to be present.
...
android {
    ...
    buildTypes {
        release {
            ...
            // fall back to the debug key so `flutter build` still works without
            // it. A build uploaded to Google Play MUST use the release key —
            // a debug-signed AAB is rejected.
