...
| 3 | Details | First name, last name, phone — all validated |
...
```
lib/
  main.dart                 App entry, providers, text-scale clamp
  models/
    appointment.dart        Booking record + JSON round trip
    salon_service.dart      Fixed service catalogue
  providers/
  ...
  utils/formatting.dart     One place for every date/time shape
```
...
- **Unit** — appointment JSON round trip, 2-hour duration, upcoming/past
  ...
  empties everything), storage persistence and corruption recovery, date
  formatting.
- **Widget** — the splash leads into the app; "Select Time" stays disabled
  until a day is picked.
- **Render** — all 11 screens at three phone sizes (360×640, 390×844, 430×932)
  and both ends of the supported text-scale range. A `RenderFlex` overflow
  surfaces as a thrown exception in widget tests, so these are a real check
  that the responsive layout holds.
